...
with tabs[0]:
    ...
    if pol.empty:
        ...
        # ── Horizontal bar: categories ranked by median views ──
        if not cat_perf.empty:
            fig = px.bar(
                cat_perf.sort_values("Median views"),
                x="Median views", y="Categories", orientation="h",
                color="Avg engagement %", color_continuous_scale="Reds",
                text="Videos",
                labels={"Median views": "Median views", "Categories": "",
                        "Avg engagement %": "Eng %"},
                height=max(300, 40 * len(cat_perf)),
            )
            fig.update_traces(texttemplate="%{text} vids", textposition="outside")
            fig.update_layout(margin=dict(l=10, r=10, t=10, b=10),
                              coloraxis_colorbar=dict(title="Eng %"))
            st.plotly_chart(fig, use_container_width=True)
            st.caption("Longer bar = more views for that topic. Darker = higher engagement. "
                       "Label = how many videos (a big bar off few videos is less reliable).")

        cat_show = cat_perf.copy()
        cat_show["Median views"] = cat_show["Median views"].round().astype(int).apply(humanize)
        cat_show["Avg engagement %"] = cat_show["Avg engagement %"].round(2)
        with st.expander("See the numbers as a table"):
            st.dataframe(cat_show, use_container_width=True, hide_index=True)

        # ── Heatmap: category × month (what's heating up) ──
        st.markdown("**Category momentum — median views by month**")
        heat_src = exploded_f.dropna(subset=["Categories"]).copy()
        if not heat_src.empty:
            pivot = (heat_src.groupby(["Categories", "Month"])["Views"].median()
                     .reset_index())
            if pivot["Month"].nunique() > 1:
                hm = px.density_heatmap(
                    pivot, x="Month", y="Categories", z="Views",
                    color_continuous_scale="Reds",
                    labels={"Views": "Median views"},
                    height=max(300, 40 * heat_src["Categories"].nunique()),
                )
                hm.update_layout(margin=dict(l=10, r=10, t=10, b=10))
                st.plotly_chart(hm, use_container_width=True)
                st.caption("Rows = topics, columns = months. Darker cell = that topic pulled "
                           "more views that month. A row getting darker left→right is heating up.")
            else:
                st.caption("Need more than one month of data for the heatmap.")
...
with tabs[1]:
    ...
    if iv.empty:
        ...
        # ── Bar: which channels' interviews pull the most (median views) ──
        by_ch_chart = (iv.groupby("Channel")["Views"].median().round().astype(int)
                       .reset_index().sort_values("Views"))
        if len(by_ch_chart) > 1:
            figc = px.bar(by_ch_chart, x="Views", y="Channel", orientation="h",
                          color="Views", color_continuous_scale="Blues",
                          labels={"Views": "Median views", "Channel": ""},
                          height=max(300, 38 * len(by_ch_chart)))
            figc.update_layout(margin=dict(l=10, r=10, t=10, b=10), coloraxis_showscale=False)
            st.plotly_chart(figc, use_container_width=True)
            st.caption("Which channels get the most out of interviews. Study the top few for "
                       "format cues (guest type, thumbnail, length).")
